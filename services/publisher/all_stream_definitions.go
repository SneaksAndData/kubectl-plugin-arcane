...
func NewAllStreamDefinitionsPublisher(provider cmdinterfaces.ClientProvider) *AllStreamDefinitions {
...
func (a AllStreamDefinitions) PublishStreamDefinitions(ctx context.Context, target interfaces.Queue) error {
	...
	for _, sc := range streamClasses.Items {
		queuePublisher := NewStreamClassMembersPublisher(a.provider, sc.Name, "", filter.NewAllowAll())
